...
create_bar_graph=function(ind_factors, dep_factor, dataset, string){
  ...
  a  
  ggsave(a, file=paste0("resultsCheck/barplot_for_", string,"_of_",dep_factor,"_by_", ind_factors[1], "_and_", ind_factors[2], ".tiff"), width=6, height=4, compression = "lzw")
}
...
create_threat_augmentatio_table=function(ind_factor, dataset, string){
  ...
  filename=paste0("resultsCheck/climateThreatAugmentationTable_for_", string,"_grouped_by_", ind_factor, ".csv")
...
multiHistFx=function(histDf, xcol, fillcol, txtID){
  mean_vals=aggregate(histDf[,xcol], list(histDf[,fillcol]), mean)
  #mean_vals=data.frame(mean_vals, cols=letters[1:(dim(mean_vals)[1])])
  a=ggplot(histDf, aes(get(xcol), ..count.., fill = get(fillcol))) +
    geom_histogram(binwidth=1, alpha=1, position="dodge")+xlab("Number of non-climatic threats")+ylab("Count")+
    geom_vline(data = mean_vals,aes(xintercept=x, colour=Group.1))
  b=ggplot(histDf, aes(get(xcol), ..density.., fill = get(fillcol))) +
    geom_histogram(binwidth=1, alpha=1, position="dodge")+xlab("Number of non-climatic threats")+ylab("Density")+  
    geom_vline(data = mean_vals,aes(xintercept=x, colour=Group.1))
  a=a+theme(legend.title=element_blank())
  b=b+theme(legend.title=element_blank())
  ggsave(a, file=paste0("resultsCheck/count_hists/hist_for_", txtID,"_of_",xcol,"_counts_by_", fillcol, ".tiff"), width=6, height=4, compression = "lzw")
  ggsave(b, file=paste0("resultsCheck/hist_for_", txtID,"_of_",xcol,"_density_by_", fillcol, ".tiff"), width=6, height=4, compression = "lzw")  
}
