...
modelGLM=glm(all_data_combined$n_threats ~ all_data_combined$CC_threat*all_data_combined$status*all_data_combined$Red.List.status*all_data_combined$Kingdom,poisson)
summary(modelGLM)

modelGLM1=update(modelGLM,~.-all_data_combined$status)
summary(modelGLM)

anova(modelGLM,modelGLM1,test="Chi")
...
#Additive (step-wise) Models from basic two way to all inclusive interactive:

model6=glm(all_data_combined$n_threats ~ all_data_combined$CC_threat,poisson)
summary(model6)

model7=glm(all_data_combined$n_threats ~ all_data_combined$CC_threat+all_data_combined$Kingdom,poisson)
summary(model7)

anova(model6,model7,test="Chi")

model8=glm(all_data_combined$n_threats ~ all_data_combined$CC_threat+all_data_combined$Kingdom+all_data_combined$Red.List.status,poisson)
summary(model8)

anova(model7,model8,test="Chi")

#NO ANOVA FOR 8 AND 9 DUE TO MATRIX INCOMPATABILITY
model9=glm(all_data_combined$n_threats ~ all_data_combined$CC_threat+all_data_combined$Kingdom+all_data_combined$Red.List.status:all_data_combined$status,poisson)
summary(model9)

model10=glm(all_data_combined$n_threats ~ all_data_combined$CC_threat:all_data_combined$status+all_data_combined$Kingdom+all_data_combined$Red.List.status,poisson)
summary(model10)

anova(model9,model10,test="Chi")

model11=glm(all_data_combined$n_threats ~ all_data_combined$CC_threat+all_data_combined$Kingdom:all_data_combined$status+all_data_combined$Red.List.status,poisson)
summary(model11)

model12=glm(all_data_combined$n_threats ~ all_data_combined$CC_threat:all_data_combined$status+all_data_combined$Kingdom:all_data_combined$status+all_data_combined$Red.List.status,poisson)
summary(model12)
anova(model11,model12,family="Chi")
